...
%\part{start}
    ...
    \section{Problem Statement}
        ...
        \begin{enumerate}
            \item 
            In contrast to the monolithic application \cite[p.~94]{{softwareDesign}} where there is only one database for the whole system, 
            every Microservice in the MARS System owns its personal database which should be accessed only by itself in order for it to be able to scale independently.
            ...
            \item 
            The design of MARS system for communication is only via remote network call like HTTP\footnote{HTTP: Hyper Text Transfer Protocol} 
            ...
            change in data one database is unknown to the other service since an ACID transaction does not existing, which can lead to archive of false data.
            ...
            \item
            The system in addition also has operational complexities due to the fact a lot of services have to be managed and it gets very hard to run all the services
            in a local enviroment to run tests i.e. integration, end-to-end tests.    
