...
exception class. This test will ensure that some method will not just consume an exception when an unexpected error occurs. This is important because if 
...
as there are no children resources (See Figure \ref{fig:marsDependency}). Listing \ref{lst:scenarisEmpty} shows an example that, if the scenario client returns a null 
